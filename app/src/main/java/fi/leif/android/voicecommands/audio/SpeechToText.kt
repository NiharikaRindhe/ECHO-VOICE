...
class SpeechToText(val listener: SpeechToTextListener) {
    ...
    fun start(context: Context, extraLang: String) {
        ...
        // Restrict language to only English (en-US) and Hindi (hi-IN)
        val allowedLanguages = listOf("en-US", "hi-IN")
        val selectedLanguage = if (allowedLanguages.contains(extraLang)) extraLang else "en-US" // Default to English
        speechRecognizerIntent.putExtra(
